...
class SiteController extends Controller
{
     ...
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function searchArticle($volume, $issue, $year, $publish_number)
    {
        if (($model = Article::find()
        ->innerJoin('jeb_journal', 'jeb_journal.id = jeb_article.journal_id')
         ->where(['jeb_journal.volume' => $volume, 'jeb_journal.issue' => $issue, 'jeb_article.publish_number' => $publish_number, 'YEAR(journal_at)' => $year])
        ->one()) !== null) {
            return $model;
        }
